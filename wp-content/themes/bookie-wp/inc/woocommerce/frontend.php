...
function toko_wc_setup_shop_page() {
	...
	if( toko_get_option( 'wc_shop_result_count_disable', true ) ) {
		remove_action( 'woocommerce_before_shop_loop', 'woocommerce_result_count', 20 );
	}

	/* Shop Page - Catalog Ordering */
	if( toko_get_option( 'wc_shop_catalog_ordering_disable', true ) ) {
	...
	if( ! toko_get_option( 'wc_shop_button_detailbuy_disable' ) ) {
...
function toko_custom_add_to_cart_text() {
	$text = toko_get_option('wc_shop_button_buy_text');
	if ( !$text ) $text = esc_html__( 'Buy', 'bookie-wp' );
	return $text;
}
...
function toko_add_detail_button() {
	$text = toko_get_option('wc_shop_button_detail_text');
	if ( !$text ) $text = esc_html__( 'Detail', 'bookie-wp' );
	echo '<a href="' . get_the_permalink() . '" class="button product-button">' . $text . '</a>';
}
