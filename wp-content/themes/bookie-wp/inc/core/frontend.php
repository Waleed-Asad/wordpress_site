...
add_action( 'wp_head', 'toko_output_style', 25 );
function toko_output_style() {
	$style = apply_filters( "toko_style", '' );
	if ( $style ) {
		echo '<style type="text/css">'."\n".$style."\n".'</style>'."\n";
	}
}

add_filter( 'toko_style', 'toko_style_customize' );
function toko_style_customize( $style ) {
	...
	if ( ! empty( $fields ) ) {
		foreach ( $fields as $field ) {
			...
			if ( in_array( $field['type'], array( 'color', 'slider' ) ) && $field['setting'] && !empty( $field['style'] ) ) {
				if ( $value = toko_get_option( $field['setting'], $field['default'] ) ) {
